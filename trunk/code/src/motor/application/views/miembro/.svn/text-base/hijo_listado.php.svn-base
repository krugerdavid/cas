...
<div class="miembro-panel">
    ...
    <div class="sc-hdr">
        <h2 class="hj-lst"><?php echo $titulo_form ?></h2>
        <div class="clear"></div>
    </div>
    ...
    <?php if(!empty($query)):?>
		...
		<div class="header">
			...
			<div class="col-hijo-options">Opciones</div>
			<div class="clear"></div>
		</div>

        <ul class="list-row">

		<!-- Listar los hijos del miembro -->
		<?php $count = 0; foreach ($query as $row): if($count % 2){ $clase = "linea1"; }else{ $clase = "linea2"; }?>
            <li class="<?php echo $clase; ?>">
                ...
                <?php if (isset($hijos_miembros[$row->prs_id])){?>
                ...
                <?php }else{ ?>
                    <?php if ($this->myutils->mayor_de_edad($row->prs_fecha_nacimiento)){?>
                       <a  href="#" onclick="<?php echo 'hijo_a_miembro('.$miembro_id.','.$row->prs_id.')';?>" class="hijo-a-mmb-item">A Miembro</a>  |
                    <?php } ?>
                    <a href="<?php echo base_url();?>miembro/hijo_editar/<?php echo $miembro_id.'/'.$row->prs_id;?>/" class="editar-item">Editar</a>  |
                <?php } ?>
                    <a href="<?php echo base_url();?>miembro/hijo_eliminar/<?php echo $miembro_id.'/'.$row->prs_id;?>/" class="eliminar-item">Eliminar</a>
				</div>
				<div class="clear"></div>
			</li>
	...
	<?php else: ?>
        <div class="empty-database">
            El miembro no cuenta con hijos.
        </div>
    <?php endif; ?>
	</div>
</div>
