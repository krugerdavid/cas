...
function otro_a_miembro(miembro, hijo)
{
   if (window.confirm("Esta seguro de asignar como miembro a este familiar?")){
      window.location = base_url + 'miembro/otro_a_miembro/' + miembro + '/' + hijo ;
   }
   return false;
}
...
$(document).ready(function(){
    ...
    $("#usr-form").validate({
        rules: {
            ...
            usr_doc_num: {maxlength: 9, regex: "^[0-9]+$", min: 1},
            //usr_fecha_nac: {required: true, regex: "^[0-9]{4}-[0-9]{2}-[0-9]{2}$"},
            usr_fecha_nac: {fecha_valida: true, fecha_mayor: true},
        ...
        messages: {
            ...
            usr_fecha_nac: {required: "Este campo no puede ser vacio.",
                        ...
                        fecha_mayor: "La persona debe ser mayor a 18 años."},
        ...
        submitHandler: function(form) {
            if ($("#usr_doc_num").val() != ''){
                $.post("http://localhost/cas/miembro/verificar_documento/",
                    { documento_numero: $("#usr_doc_num").val(), miembro_id: $("input[name='miembro_id']").val()},
    ...
    $("#bt_cancel").click(function(){
       $(':input','#usr-form')
            ...
            .removeAttr('selected');
       $("input[name='usr_bautizado']:nth(0)").attr('checked', 'checked');
       $("input[name='usr_confirmado']:nth(0)").attr('checked', 'checked');
